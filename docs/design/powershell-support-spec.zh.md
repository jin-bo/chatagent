...
为底；**相对 rev 24 的每一处语义偏离都列在评审记录的 rev 25 至 rev 47 行**，不在这里转述。四十一轮评审、三百零八条发现的记录在评审记录文件。**独立于本规范的 PR-0**（子代理没有权限引擎，一处已实测的活缺陷）
...
| **ENV-06g** | **两类的判据是「有没有规则依赖这个目录的内容」，不是这个键叫什么：** 系统那一类是解释器与工具链**从中加载**或**从中读配置**的根（系统根、程序目录、`ComSpec`），须过 IMG-01；profile 那一类是主体自己的数据目录，按定义主体可写、只查形态。按这条判据归到**后者**的有三个，都是实测定的：`PUBLIC` 是共享用户数据目录；`ProgramData`／`ALLUSERSPROFILE` 无消费者 —— git 只读一个配置文件且在程序目录下，标准用户种得下 `ProgramData\Git\config` 而 git 忽略它（§3.24）；`SystemDrive` 的卷根没有东西从中加载，且它已在**每一条**祖先链上按祖先掩码求过值。 | 把设计上可写的目录放进「须过 IMG-01」那一类，IMG-06a **目标掩码**里的 ADD 两位就足以让 `attested_spec` 拒掉**每一个**政策开启的 rung，翻转后每次 shell 调用 DENY —— 三个键各自单独都做得到这件事（§3.23、§3.24）。按名字读起来像什么来归类，是这里出过三次的同一个错 | §3.21、§3.22、§3.23、§3.24 |
...
| **LADDER-05** | 翻转之前（PR-1 至 PR-6）Windows 的默认执行器报 `CMD × legacy_cmd`：`%COMSPEC% /c`、今天的环境、今天的（空转的，§2.4）regex 地板，裁定与 `main@3537753` 逐段相同；它不是阶梯的一级，阶梯只在翻转后运行。PR-7 删除这个取值，之后报它的 spec 按 SPEC-02 拒绝。**阶梯开不开可由用户级 `shell.ladder` 读回**（G09-02）——未设跟随内建常量，显式取值胜过它，`ladder_enabled` 是两个读者唯一的合并点 | 没有这个值，翻转前的 Windows 要么在 PR-2 就被翻转（`CMD × cmd`），要么每次调用 DENY（`UNKNOWN`），要么用 POSIX 模式扫 cmd 文本报干净地板（`system_posix`）—— 三条各违反一条规则 | §2.1、§2.4 |
| **LADDER-04** | 翻转（PR-7）的前提：G09 的三项逐条绿 —— 日常集在政策开启的 cmd 与 PowerShell 下仍 PASS、`LADDER_FLIPPED` 能由配置读回（退路不靠发版）、每次拒绝带可计数的理由串 —— 再加 `ruff` 绿；Git Bash rung 只在 G20 绿时启用，红则关着这一级发布。**翻转分两步**：先让阶梯**可达**（`shell.ladder: true` 可选入，默认不动），再改默认 —— 因为实测下提权的 Windows token 可信集为空、LADDER-03 拒掉每一次 shell 调用（§3.23），而在 Windows 上提权运行很常见：一步到位等于让那批用户先撞墙再去找退路 | 不为 Windows 声称没在 Windows 上测过的东西。而「可接受的降级分布」是关于用量的问题，翻转前无人能答：§7.2 把「不透明桶不可用」列为改变本规范的条件，也就是说那个数字来自发布之后。所以这道门收可证伪的三项加一条退路，不收一个猜出来的百分比 | §2.5 |
...
**2026-09-05 定案 —— PR-2 之前的五道门与 PR-4 之前的两道门全部关闭。** 四道由用户在 PR-1 落地时定（q4、q9、q2/q3、q13），q11 判为不可观测，q14 由两次实测定（§3.23 的 ACL 半边、§3.24 的消费者半边）：
...
| q14 | **已关：`ProgramData`／`ALLUSERSPROFILE` 与 `SystemDrive` 一并按 ENV-06g 的判据归到只查形态那一类。** 三个备选里「保留并接受阶梯走空」被实测排除，「逐键弱谓词」不必要 | 两次测量各答一半：`C:\ProgramData` 对标准用户给 `FILE_ADD_FILE`（§3.23），但**没有消费者** —— git 只读一个配置文件且在程序目录下，标准用户种得下 `ProgramData\Git\config` 而 git 忽略它，可信表里别的程序在那儿连目录都没有（§3.24）。同一次清扫发现卷根也在这一组里、也单独致命，而它**已经**在每条祖先链上按祖先掩码求过值 —— 按名字归类的同一个错，`PUBLIC` 之后的第二和第三例。绊线 G23-15 |
...
14. **出厂 Windows 上，`C:\ProgramData` 过得了 IMG-01 吗？** **已关，2026-09-05：过不了，而且不需要它过。**
    ACL 那一半见 §3.23，消费者那一半见 §3.24；按 ENV-06g 的判据它与 `ALLUSERSPROFILE`、`SystemDrive` 一并
    归到只查形态那一类，见上面的决策表 q14 行与 G23-15。以下是原文。它与 `ALLUSERSPROFILE` 按 ENV-06g 的判据留在系统那一类

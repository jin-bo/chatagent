...
| G04-39 | EFF-07、CMD-01、WRAP-05 | cmd body `call foo.bat` 与 `start x.exe`（各再测一次跟在分隔符之后） | 前者拒于 `hardline:cmd-opaque:CMD-01:call`，后者拒于 `hardline:cmd-opaque:WRAP-05:start`；两者都在 body 被切成命令**之前**被拒，因此都到不了效果表 —— 这是 §7.3 q11 关闭所依赖的可达性，谁把 `start` 移出生成进程集或把 `call` 移出控制关键字集，本行变红、q11 自动重开 | `CMD-01:call` / `WRAP-05:start` | ubuntu / PR-2 |
...
| G09-01 | LADDER-04 | 规范 §7.3 q9 那档日常工具链，逐条跑过**政策开启**的 cmd spec 与 PowerShell spec | 全部 PASS；任一条不透明即红 —— 翻转后 `git status` 变不透明，没有哪个百分比救得回来 | — | ubuntu / PR-7 |
| G09-02 | LADDER-04、LADDER-05、CFG-01 | **已实施。** 用户级 `shell.ladder` 关→开→关，每次重新导出 spec；另把内建常量置真再用配置取回；再验未设 ≠ 关 | 三次分别是翻转前 rung、阶梯选出的 rung、翻转前 rung，两端指纹相等；内建为真时未设跟随它而显式 `false` 夺回；`ladder=None` 与 `ladder=False` 是两个答案 —— 读作同一个会让发布日谁都收不到 | — | ubuntu / PR-7 |
| G09-03 | LADDER-04 | **已实施。** 一批会被拒的 body 跑过政策关闭与政策开启两种 spec，逐条分类并聚合；另把包内**每一条** reason 字面量也过一遍分类器 | 六个族全部识别，`unknown` 为空；聚合键比原串粗（细节里有路径与命令词，按原串计数等于一次调用一行），但不塌 —— **这道门收的是仪器在场，不是事前猜出来的百分比**。**原判据「每条 reason 都是 `hardline:<dialect>-opaque:<规则 ID>[:<细节>]`」是错的**：那只是六族之一，另有 23 条是 POSIX 危险表的自由英文（`hardline:recursive delete of root / system directory / home`），冒号与空格在那里不表示结构 | `unknown:<原串>` | ubuntu / PR-7 |
| G09-05 | LADDER-04、CFG-01、SPEC-05 | **已实施。** `LocalShellExecutor` 分别以「无块」「`ladder=true`」「`ladder=false`」三种构造读 `shell_spec`；另经 `build_from_environment` 从用户级文件走一遍 | 三个答案里开启那个与另外两个**不同**（一个有消费者的取值长这样）；工厂造出的执行器确实带着它加载的那个块 —— 没有这一步，`shell.ladder` 有解析器、有校验，却谁都读不到 | — | ubuntu / PR-7 |
| G09-04 | LADDER-04 | `uv run ruff check .` | 绿 | — | ubuntu / PR-7 |
...
| G23-15 | ENV-06g、ENV-06a、IMG-06a | 桩 oracle 按实测出厂 ACL 作答（`C:\`、`C:\ProgramData`（键 `ALLUSERSPROFILE`）、`C:\Users\Public` 主体**能建条目、不能删改**，其余干净），逐个单独一遍再合起来一遍；反向再来一遍：`C:\Windows`、`C:\Program Files`、`ComSpec` 各自可写 | 前者六次都导出 `ShellSpec`（`SystemDrive` 的卷根、`ProgramData` 与 `PUBLIC` 都不再逐项过 IMG-01）；后者三次都 `Exhausted` —— 收窄之后这道检查仍然咬人，不是变成了摆设 | `ENV-06: pinned system dir` | ubuntu / PR-4 |
...
翻转前收三项，每一项都能红：日常集在政策开启下仍 PASS；`LADDER_FLIPPED` 能由配置读回，于是退路不靠发版；每次拒绝带可计数的理由串。`uv run ruff check .` 绿。

**这三项替掉的是「三个桶的降级率经接受」。** 那句话在整个文件集里出现四处，四处都是它自己的复述，三个桶从未被列举，没有阈值、没有语料、没有验收人，证据文件里为它量过的东西是零 —— 一道谁都判不了红的门不是门。它想抓的东西是真的（翻转后多少日常工作变成一次弹窗），但那是关于**用量**的问题：规范 §7.2 把「不透明桶不可用」与「实测的 Windows 用户数为零」都列为会改变本规范的条件，也就是说那个数字只可能来自发布之后。所以分布由 G09-03 的理由串在发布后量，翻转前收的是 G09-01 的可证伪断言加 G09-02 的退路。

...
| G09-02 | LADDER-04 | 由配置把 `LADDER_FLIPPED` 关→开→关，每次重新导出 Windows 的默认 spec | 三次分别导出翻转前的 rung、阶梯选出的 rung、翻转前的 rung；退路不需要改代码、也不需要发版 | — | ubuntu / PR-7 |

...
| G09-03 | LADDER-04 | 一批会被拒的 body，逐条取它的 reason 串并按串聚合 | 每条 reason 都是 `hardline:<dialect>-opaque:<规则 ID>[:<细节>]` 形状，聚合即得分布 —— 这道门收的是**仪器在场**，不是某个事前猜出来的百分比 | — | ubuntu / PR-7 |

...
而 rev 19 是一遍编辑性整理、rev 32 与 rev 42 是结构修订、rev 45、rev 46、rev 50、rev 51 与 rev 52 是实施轮、rev 47 与 rev 49 是实测定案、rev 48 是一次定案轮，都不是一轮评审** —— 这就是「rev 2–52 那五十一行、合计三百四十五条」与「表头写
...
| rev | 发现 | 错在哪一类 | 规则现在在 |
|---|---|---|---|
| 52 | 3 条 | **PR-7 第一步的实施轮，不是一轮评审 —— 而它开工前先改了 PR-7 自己的范围。** (1) **翻转改为分两步，这是范围决定不是实现细节。** 证据 §3.23 实测：提权的 Windows token 可信集**为空**，LADDER-03 于是拒掉每一次 shell 调用；而在 Windows 上提权运行很常见。一步到位改默认，等于让那批用户**先撞墙、再去找退路**，而退路（`shell.ladder: false`）rev 50 才刚建好。**这也正是 rev 48 重写 G09 时那句话的推论**：门收的是仪器与退路、分布在发布之后量 —— 那么先发的应当是**能选入的能力**，不是新默认。用户定案：先可达、后翻转。 (2) **`shell.ladder` 此前有解析器、有校验、没有消费者。** 工厂加载 CFG-03 的块之后没有任何一处读它（源码注释自己写着「Nothing reads it yet」），而 `default_spec()` 造的是空 `ShellBlock()` —— **G09-02 建的那个退路，在进程里并不存在**。接线：`default_spec(config, …)`、`LocalShellExecutor(shell_block=…)`、工厂在宿主没给执行器时替它造一个。 (3) **两处主体拼法在 Windows 上不一致，差点让 oracle 拒绝自己被问的每一个问题。** `local_subject()` 在 Windows 上回退到**登录名**，而 `native_oracle()` 讲的是 **SID**；SPEC-05 把每个答案绑定到一个主体，两者不等则 `_for()` 全数拒答。改为用 `token_sid()` 取一次、同一个值同时交给 oracle 与 `select_rung`。**另一条同族的**：我第一版把 `token_sid()` 写在`native_oracle()` 之前，而前者是裸 `ctypes`、没有平台守卫 —— 在 POSIX 主机上以 Windows 为目标会抛 `AttributeError`，而**异常不是裁定**（方法规则 22）。守卫要放在**两次调用之前**，不是之间。 | `LADDER-04`；门槛 G09-05；`agentao/capabilities/shell_spec.py::default_spec`、`capabilities/shell.py`、`embedding/factory.py`；实现文件 PR-7 行 |

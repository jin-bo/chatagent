...
> 本文件收**历史与方法**：四十轮评审的修订表、每轮发现的类别，以及这些轮次产出的二十九条方法规则。
...
而 rev 19 是一遍编辑性整理、rev 32 与 rev 42 是结构修订、rev 45、rev 46 与 rev 50 是实施轮、rev 47 与 rev 49 是实测定案、rev 48 是一次定案轮，都不是一轮评审** —— 这就是「rev 2–50 那四十九行、合计三百三十八条」与「表头写
...
| rev | 发现 | 错在哪一类 | 规则现在在 |
|---|---|---|---|
| 50 | 1 条 | **G09-02 的实施轮，不是一轮评审。** 翻转开关改为可由用户级 `shell.ladder` 读回，`ladder_enabled(config)` 是**唯一**的合并点（`select_rung` 与 `default_spec` 两个读者，两处各自解析同一个标志正是方法规则 26 点名的缺陷）。**这一条的全部设计负担落在一个取值上：`None` 不是 `False`。** 用普通 `bool`，「没配过」与「关掉了」就是同一个值，于是 `LADDER_FLIPPED` 置真那天，每一台没配过的主机都会静默地留在旧行为里 —— **发布等于发给零个人，而且没有任何配置说得出为什么**。所以未设为 `None`、跟随内建，显式取值两个方向都胜过内建；加载器里 `raw.get("ladder")` 为 `None` 时不折成 `False`。**经配置打开在发布前不受支持、且可能拒掉每次 shell 调用**（LADDER-03），不加护栏：同一个键就是退路，而只有发版够得着的退路不叫退路。测试三条，其中两条把内建置真再用配置夺回 —— 只在内建为假时断言，会漏掉唯一真正要紧的那种情形：发布之后 shell 被拒的那个用户。 | `LADDER-05`、`CFG-01`；门槛 G09-02；`agentao/capabilities/shell_spec.py::ladder_enabled`、`embedding/permission_loader.py`、契约模块；`docs/reference/configuration.md` |

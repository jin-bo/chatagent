...
> 本文件收**历史与方法**：四十一轮评审的修订表、每轮发现的类别，以及这些轮次产出的三十条方法规则。
...
而 rev 19 是一遍编辑性整理、rev 32 与 rev 42 是结构修订、rev 45、rev 46、rev 50、rev 51 与 rev 52 是实施轮、rev 47 与 rev 49 是实测定案、rev 48 与 rev 54 是定案轮，都不是一轮评审** —— 这就是「rev 2–54 那五十三行、合计三百五十七条」与「表头写
四十一轮评审、三百零八条发现」之间差别的全部（rev 25 的十五条、rev 26 的二十四条、rev 27 的十二条、rev 28 的十条、rev 29 的十条、rev 30 的九条、rev 31 的八条、rev 33 的六条、rev 34 的十五条、rev 35 的九条、rev 36 的六条、rev 37 的五条、rev 38 的两条、rev 39 的两条、rev 40 的两条、rev 41 的三条、rev 43 的十七条、rev 44 的三条、rev 53 的七条都在表里）。谁要重算这两个数，需要的是这条约定，不只是算术；而
...
| rev | 发现 | 错在哪一类 | 规则现在在 |
|---|---|---|---|
| 54 | 4 条 | **一次定案轮，不是一轮评审 —— 方向从「补严格模型的洞」改为「把严格模型摘出默认路径」，方案落在 `powershell-support-lightweight.zh.md`。** (1) **转向轻量实现，从当前 HEAD 做减法，不对 main 做硬回退。** 理由是提交本身混着三样东西：PR #213 一次带进了基础 PowerShell 支持、严格认证策略，以及 Windows job 首跑抓出的那批缺陷修复（CRLF、状态目录、sqlite、会话、`os.replace`、ACP 窗口、内存关闭）—— 第三样与 PowerShell 无关，整体回退会把它们一起丢掉。`682bd3a`（PR #212）停在大实现之前，作**审查基线**用来对照哪些属于基础支持，但它自己不含任何 Windows 修复，不是恢复目标。**没有紧急压力**：默认仍是 `%COMSPEC% /c`，严格阶梯要选入才走。 (2) **档位从两态改三态，而且档位与 rung 正交。** 今天 SPEC-03 让 `policy_enabled` 等于「rung 不在 `POLICY_OFF_RUNGS` 里」，于是只有「今天的 cmd 加正则地板」与「可信根、封闭集、认证、钉值环境全套」两种可能，中间没有东西 —— 而「让 agentao 在 Windows 上正确使用 PowerShell」要的正是中间那一档。拆成 `legacy` / `basic` / `strict`：rung 回答选中了哪个解释器（SPEC-02 本来就是这么写的），档位回答加多少政策。 (3) **`basic` 的分界判据：只因看见危险类而拒，绝不因读不懂而拒。** 两个扫描函数的两半在源码里已经分开 —— `scan_powershell` 是先降级再跑危险表、降级失败即拒，`scan_cmd` 是先危险表再做 TOK-02 与 CMD-01 的可读性拒绝。`basic` 只取前一半，读不懂时回落到今天默认已经在跑的通用危险表。**推论要写进规则：**`basic` 在任何输入上都不比今天的默认更弱，只可能更强 —— 没有这句话，「放宽」听起来像「放松」。 (4) **验收改为真实开发任务，而且它必须能红。** 契约测试全绿不构成验收；克隆、建虚拟环境、装依赖、构建、测试，再加「装一个用户级工具后裸名字调用它」。最后这项专冲 ENV-01：它在 `strict` 下必然红、在 `basic` 下必须绿，**两个答案都要进矩阵** —— 只记一个，门就退回成方法规则 29 说的那种不是门的东西。 | 方案文件 `powershell-support-lightweight.zh.md`（不在受检文件集内，不定义规则）；被采纳后要改的是 `SPEC-03`、`SPEC-06`、`CFG-02c`、`ENV-01`、`LADDER-04`、`LADDER-05`；`agentao/permissions_hardline/_scanner.py`、`_powershell.py`、`_cmd.py`、`_trust.py::select_rung`。**未实施。** |
| 53 | 7 条 | **一轮评审，不是实施轮 —— 用户对这套机制强度的质疑，加上答问过程中自查出的四条；七条全部指向 PR-7 第二步，全部未定案。** (1) **翻转的最后一道前提收不上来，而方法规则 29 (b) 正是写它的那一版立的。** LADDER-04 把第二步的前提写作「等 G09-03 收到的分布」，可 agentao 没有任何遥测 —— `agentao/` 里 telemetry 一词只出现在注释里，说的是宿主拿事件能做什么 —— 而 2026-08-14 的 OTel 评审结论就是不建、依赖方实测为零。于是这道门谁都过不去。规则 29 说的是判不了红的门不是门，这是它的镜像，判据补 (c)。 (2) **提权态没有任何诊断，而产品自己也测不出提权。** 读 `TokenElevation` 的代码只在 `scripts/windows_oracle_probe.py` 里，本机 oracle `_windows_identity.py` 从没查过它。翻转之后那批用户看到的是每次 shell 调用 DENY、没有一句解释、也不知道退路在哪。三条路各不相同：把这一问搬进 oracle 并在启动时明说（**诊断，不是放行** —— 用户答「继续」并不能让「这个 token 能替换掉它正要运行的东西」变假）；宿主级放行沿用现成的 `enable_hardline=False`（构造参数，不在任何 `permissions.json` 里）；**唯一真正恢复那条性质的**是用 agentao 自己的 linked non-elevated token 启动子进程 —— 整套设计文件里 grep `CreateProcessAsUser`、linked token、降权，零命中，这条从没被考虑过。 (3) **退路与删除在同一步里相撞。** LADDER-04 要求退路不靠发版，也就是 `shell.ladder: false` 必须管用；而第二步同时要删掉 `legacy_cmd`。可 `select_rung` 在 `ladder_enabled` 为假时返回的正是它，删了它，Windows 上 `ladder: false` 就没有 rung 可返回 —— 退路在要求它的那同一步里被拿掉。翻转后 `ladder: false` 该导出什么，整个文件集里没有一处说明。 (4) **这套强模型只对 Windows 生效。** §7 q4 已定案 `system_posix` 维持政策关闭，TOK-02、EFF-\*、IMG-02 只在 Windows 的政策开启级生效。同一个模型、同样的注入面、两套标准，而 Windows 恰恰是工具链最集中装在用户可写目录下的那个平台，受损最重。 (5) **缺中间那一级。** 今天只有政策关闭（`legacy_cmd`，那是 cmd 不是 PowerShell）与政策开启（可信根加封闭集）两档，「用 PowerShell、正确引用与编码、按方言扫危险类，但不要可信根与封闭集」这个配置配不出来。技术上可造，证据在扫描器里：PowerShell 分支先跑 `scan_powershell(cmd)`，它返回拒绝就直接拒，只有它没拒时才需要 decided record 去做封闭集那一半 —— 危险类与可信根本来就是解耦的。建议把第二步从「翻转成全套」改为「翻转成中间级」、全套保持选入；要动 `POLICY_OFF_RUNGS`、LADDER-04 与 LADDER-05，并给中间级补门槛。 (6) **门槛矩阵里没有一行是真实开发任务。** G09-01 那一档是只读工具链（`git status`、`ls`、`cat`、`grep` 一类），克隆仓库、装依赖、建虚拟环境、跑构建与测试一条都没有 —— 而翻转后受损最重的正是这些：ENV-01 把 PATH 过滤成子进程主体写不了的目录，Windows 上 scoop、npm 全局、cargo、pyenv、uv 与项目虚拟环境全在用户可写处。「日常集仍 PASS」量的是读，不是干活。 (7) **实现文件的状态段与 main 不符三处。** 文件头仍写「PR-7 未开工」、§5.4 与 §5.5 仍写本机 oracle「尚未落地」（PR #214 已落地）、§5.5 仍写 `default_spec` 在翻转为真时抛异常（PR #220 之后它会真的选级）；文件头停在 rev 45，而门槛矩阵已到 rev 52。 | `LADDER-04`、`LADDER-05`、`SPEC-03`（`POLICY_OFF_RUNGS`）；方法规则 29 (c)；`agentao/permissions_hardline/_trust.py::select_rung`、`_windows_identity.py`；门槛 G09-01；实现文件 §5.4、§5.5 与文件头。**七条均未定案，规范与矩阵未改。** |
| 52 | 4 条 | **PR-7 第一步的实施轮，不是一轮评审 —— 而它开工前先改了 PR-7 自己的范围。** (1) **翻转改为分两步，这是范围决定不是实现细节。** 证据 §3.23 实测：提权的 Windows token 可信集**为空**，LADDER-03 于是拒掉每一次 shell 调用；而在 Windows 上提权运行很常见。一步到位改默认，等于让那批用户**先撞墙、再去找退路**，而退路（`shell.ladder: false`）rev 50 才刚建好。**这也正是 rev 48 重写 G09 时那句话的推论**：门收的是仪器与退路、分布在发布之后量 —— 那么先发的应当是**能选入的能力**，不是新默认。用户定案：先可达、后翻转。 (2) **`shell.ladder` 此前有解析器、有校验、没有消费者。** 工厂加载 CFG-03 的块之后没有任何一处读它（源码注释自己写着「Nothing reads it yet」），而 `default_spec()` 造的是空 `ShellBlock()` —— **G09-02 建的那个退路，在进程里并不存在**。接线：`default_spec(config, …)`、`LocalShellExecutor(shell_block=…)`、工厂在宿主没给执行器时替它造一个。 (3) **两处主体拼法在 Windows 上不一致，差点让 oracle 拒绝自己被问的每一个问题。** `local_subject()` 在 Windows 上回退到**登录名**，而 `native_oracle()` 讲的是 **SID**；SPEC-05 把每个答案绑定到一个主体，两者不等则 `_for()` 全数拒答。改为用 `token_sid()` 取一次、同一个值同时交给 oracle 与 `select_rung`。**另一条同族的**：我第一版把 `token_sid()` 写在`native_oracle()` 之前，而前者是裸 `ctypes`、没有平台守卫 —— 在 POSIX 主机上以 Windows 为目标会抛 `AttributeError`，而**异常不是裁定**（方法规则 22）。守卫要放在**两次调用之前**，不是之间。 (4) **第四条又是 Windows job 抓的，而这次代码是对的、测试是错的 —— 方法规则 28 的对偶。** 我新写的两条测试断言 `"oracle" in reason`，那是**本机（POSIX）**的答案；在 Windows 上 `native_oracle()` 造得出来、阶梯**真的跑了**，然后因为 runner 是提权管理员而拒掉每一级，理由是 `every rung refused`。**两个都是这个函数在正常工作。** 规则 28 (b) 说的是「在缺陷不发作的平台上跑绿，只证明守卫不咬人」；这一条是它的另一面：**在某个平台上写下的断言，会把那个平台的答案当成规则本身**。改法是断言**通道**（是裁定不是异常，这才是真正的主张），理由按 host 分支；另一条名字里就写着「on a posix host」的测试改为在 Windows 上 skip —— 那里没有 POSIX 主机可测。 | `LADDER-04`；门槛 G09-05；`agentao/capabilities/shell_spec.py::default_spec`、`capabilities/shell.py`、`embedding/factory.py`；实现文件 PR-7 行 |
| 51 | 3 条 | **G09-03 的实施轮，不是一轮评审 —— 而清点第一件事就把这道门自己的判据推翻了。** (1) **G09-03 原话「每条 reason 都是 `hardline:<dialect>-opaque:<规则 ID>[:<细节>]`」是错的。** 机检包内全部 reason 字面量：**六个族**，那只是其中一族（27 条）；另有 Windows 危险表 10 条裸类名、**POSIX 危险表 23 条自由英文**（`hardline:recursive delete of root / system directory / home` —— 冒号与空格不表示结构）、launch-attest 14 条、走空 rung 与 spec 各若干。**照原判据写分类器，会把大多数拒绝随手分错桶** —— 而一个渲染得很干净、却错得没人报的数字，正是分布这种东西活不下来的方式。两个危险族因此按**表自身的成员关系**识别，不按形状：往表里加一条不会造出 unknown，改一句措辞也不会把它悄悄挪桶。 (2) **`opaque()` 渲染的是 `{detail or rule}` —— 带了细节，规则 ID 就整个不出现在串里。** 所以「按规则聚合」对一大批真实拒绝根本做不到；我第一版的 bucket 键写成 `rule or "?"`，会把 `executes_input`／`name`／`image` 塌成同一个桶。改成 `rule or detail`。**不动 reason 词表**：那个前缀是有文档的公开事件契约，为一道测量门去改 29 条字面量是本末倒置，把限制写下来即可。 (3) **自查一条，被我自己的断言抓回：** 第一版语料只跑政策关闭的 spec，而 LADDER-05 保证那两级不跑新地板 —— 于是十条 body 只产出危险表一族，`test_..._reads_as_a_distribution` 直接变红。**「我的语料覆盖了六族」这句话，在我写下它的时候是假的**；改成同时跑一条政策开启的 `cmd` spec。语料**不声称有代表性** —— 有代表性的语料正是还不存在的那个东西，假装有就是这道门被替换掉的原因。 | 门槛 G09-03；`agentao/permissions_hardline/_refusals.py`（`classify_refusal`／`tally`，经包 `__all__` 导出给宿主）、`tests/test_refusal_tally.py` |
| 50 | 2 条 | **G09-02 的实施轮，不是一轮评审。** 翻转开关改为可由用户级 `shell.ladder` 读回，`ladder_enabled(config)` 是**唯一**的合并点（`select_rung` 与 `default_spec` 两个读者，两处各自解析同一个标志正是方法规则 26 点名的缺陷）。**这一条的全部设计负担落在一个取值上：`None` 不是 `False`。** 用普通 `bool`，「没配过」与「关掉了」就是同一个值，于是 `LADDER_FLIPPED` 置真那天，每一台没配过的主机都会静默地留在旧行为里 —— **发布等于发给零个人，而且没有任何配置说得出为什么**。所以未设为 `None`、跟随内建，显式取值两个方向都胜过内建；加载器里 `raw.get("ladder")` 为 `None` 时不折成 `False`。**经配置打开在发布前不受支持、且可能拒掉每次 shell 调用**（LADDER-03），不加护栏：同一个键就是退路，而只有发版够得着的退路不叫退路。测试三条，其中两条把内建置真再用配置夺回 —— 只在内建为假时断言，会漏掉唯一真正要紧的那种情形：发布之后 shell 被拒的那个用户。**第二条由 Windows job 抓出，本机四条命令全绿也看不见：** 一条仅 Windows 的测试 `monkeypatch.setattr(_trust, "LADDER_FLIPPED", True)`，而 `_trust` 已经不再持有这个名字，于是 `AttributeError`。根因不是「我删了一个导入」，而是**原本就有两份拷贝** —— `_trust` 靠 `from … import` 拿到自己那一份，`default_spec` 读 `shell_spec` 那一份；补丁打中哪一份，取决于测试从哪个模块进，**两个读者各自只够得着一份**（方法规则 26 的同族，收敛成 `ladder_enabled` 正是修它）。改法是让那条测试按**用户真正的做法**打开开关（`ShellBlock(ladder=True)`），不再补丁模块属性 → **方法规则 30**。 | `LADDER-05`、`CFG-01`；门槛 G09-02；`agentao/capabilities/shell_spec.py::ladder_enabled`、`embedding/permission_loader.py`、契约模块；`docs/reference/configuration.md` |
| 49 | 3 条 | **一次实测定案，不是一轮评审 —— 一个被推翻的假设，一条随它掉出来的更重的缺陷。** (1) **我提的假设错了，探针推翻了它。** q14 的 ACL 半边 rev 47 已量（`C:\ProgramData` 给标准用户 `FILE_ADD_FILE`），缺的是消费者半边。我判定 git 决定这一问 —— Git for Windows 读 `%ProgramData%` 下的 system 配置，而 git config 能设 `core.pager` 与别名，这两样以 `-c` 开关出现时效果表**已经**判为执行触发器，一份种下的配置文件不带开关就到同一个地方。实测（run 34013117638）：git 2.55.0.windows.5 只读 `C:/Program Files/Git/etc/gitconfig` 一个文件；标准用户**确实**建得出 `C:\ProgramData\Git\config`，而 git 读都不读 —— **不是「文件不在所以没读」，文件是探针建的，它照样忽略**；可信表里 python／node／rg 在 `ProgramData` 下连目录都没有。于是按 ENV-06g 自己的判据，这两个键归只查形态那一类，q14 由判据自己定案，不是三选一。(2) **顺手量出来的那条更重：卷根也在这一组里，也单独致命。** `system_paths()` 把 `SystemDrive` 的盘根当**链头**递给 `trusted_root_chain`，于是它吃**目标**掩码，而目标掩码含 add-subdirectory 一位、出厂 `C:\` 恰恰授予每个标准用户 —— rev 47 为链上行走修好的那个缺陷，活在第二个消费者里。实测：三个键**各自单独**都让 `attested_spec` 返回 `Exhausted`，翻转后每次 shell 调用 DENY。卷根不必特判：它**已经**在每一条祖先链上按祖先掩码求过值。**按名字读起来像什么来归类，是 `PUBLIC` 之后的第二、第三例**（方法规则 19 的同族）。(3) **自伤一条：** 删 `drive_root` 时用「从 `def drive_root` 找到下一个 `\ndef `」定位，而类里的方法是缩进的 —— 那一刀连整个 `PinnedEnv` 类一起删了，靠改完立刻核结构（AST 数顶层名）发现并 `git checkout` 恢复。**这是 rev 15、rev 46 之后的第三例：脚本化编辑要按内容定位，不能按「下一个同类标记」扫。** | `ENV-06g`；规范 §7.3 q14；门槛 G23-15；证据 §3.24；`agentao/capabilities/shell_spec.py`、契约模块（`drive_root` 随之删除，已无调用者也无接缝欠它） |
| 48 | 4 条 | **一次定案轮，不是一轮评审 —— 两道悬着的门都靠读代码加一次本机实测关掉，而把门槛写成测试的过程自己又抓出两处文档缺陷。** (1) **q11 判为不可观测，不需要 Windows 探针。** 它问 cmd 的 `call` 与 `start` 带哪种 `rebinds_caller` 作用域，而两者都到不了那个标志被读的那一点：`call` 是 CMD-01 的控制关键字，body 被切成命令之前整体拒；`start` 在 WRAP-05 的生成进程集里，查可信表之前就拒；`rebinds_caller` 在 `_analysis.py` 里只有一个读点，且只在条目会重新进入时可达，而按 EFF-03 文件形式永不重新进入。**EFF-07 早就写着「两者到不了本条」，只是当初只对 `call` 那一半下了结论**，实现文件 §5.1 也早就并列写着那两个「今天不可达」的事实、同样没往下推一步。关闭以那两次集合成员关系为条件，绊线 G04-39。(2) **降级门判不了红 → 换掉。** LADDER-04 把翻转的前提之一写作「G09 的三个桶降级率经接受」，而「三个桶」在整个文件集里只出现四处、四处都是这句话自己的复述：从未列举，没有阈值、没有语料、没有验收人，证据文件里为它量过的东西是零。它想抓的东西是真的（翻转后多少日常工作会变成一次弹窗），但那是关于**用量**的问题，而 §7.2 自己就把「不透明桶不可用」与「实测的 Windows 用户数为零」列为会改变本规范的条件 —— 那个数字只可能来自发布之后。改成三项各自能红：日常集在政策开启下仍 PASS（G09-01）、`LADDER_FLIPPED` 能由配置读回（G09-02）、每次拒绝带可计数的理由串（G09-03），分布留给发布之后 → **方法规则 29**。(3) **q9 的惰性集列了两条按 EFF-01 定义不可能惰性的成员。** 写 G09-01 时逐条实测发现：`python -c` 与 `node -e` 在三级政策开启的 rung 上全部 DENY（`executes_input`），而 q9 的定案原文把它们列在惰性集里，G04-30 又早就把它们钉成不透明 —— 定案与门槛互相矛盾，实现跟的是门槛。代价改为进测试而不是留在散文里。(4) **EFF-07 末条比 §1 的边界宽。** 「以及任何被喂了脚本路径的解释器」照字面读会把 `python foo.py` 收进来，而 §1 明写可信工具链**按设计**跑工作树内容仍算惰性（`git commit` 跑 hooks 是同一件事）；冻结的 rev 24 原文写的是「被喂了脚本路径的 `bash`/`cmd`/`pwsh`」，实现也只拦这三个（WRAP-01–03）。**拆分时的缩写把一条限定的规则写成了不限定的**，收窄回去。 | `EFF-07`、`LADDER-04`；规范 §7.3 q9／q11；门槛 G04-39、G09-01–04；`tests/test_closed_set.py`；方法规则 29 |
...
**四十一轮评审产出了下面这些规则 —— 轮次数取自表头，不是另记一份；**规则条数则要自己数，rev 32 加了第 16 条而这句话

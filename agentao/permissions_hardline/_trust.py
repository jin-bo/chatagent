...
from ._measured_commands import MEASURED as _MEASURED
...
MEASURED_COMMAND_TABLES: Dict[Tuple[str, str], Tuple[MeasuredEntry, ...]] = {
    key: tuple(
        MeasuredEntry(name=name, kind=kind, alias_target=target or None)
        for name, kind, target in rows
    )
    for key, rows in _MEASURED.items()
}
"""NAME-02: alias / function / cmdlet rows, keyed by ``(edition, version)``.

Filled from ``_measured_commands.py``, which is generated by
``scripts/windows_command_table_probe.py`` and never hand-edited. The measurement runs inside
the pinned startup state :func:`prelude_for` builds — ``-NoProfile``, auto-loading off, the
two modules imported explicitly, the default session configuration — and every row is then
verified resolvable in that same state.

**The key is an exact build, and a miss is a working outcome rather than a gap.** An identity
with no entry gets :func:`identity_measured` ``False``, which makes every PowerShell bare word
opaque while the rung still serves explicit paths (IMG-04) — the same degradation NAME-02
already specifies for a closed environment that was not established. So a new Windows build
degrades rather than breaks, and re-running the probe is what promotes it.
